...
# Add get_all_regions import
text = text.replace("from .config import", "from .providers.aws.regions import get_all_regions\nfrom .config import")

# Find the start of scan_aws_resources
start_idx = text.find("def scan_aws_resources():")
end_idx = text.find("def heal_security_group_ssh")

new_func = """def scan_aws_resources():
    session = get_session()
