...
class OnPremAdapter(BaseCloudAdapter):
    provider_name = "onprem"

    async def describe_resource(self, resource_id: str) -> Optional[Resource]:
        logger.info("[OnPremAdapter] describe_resource: %s", resource_id)
        return None  # TODO: integrate OnPrem SDK

    async def list_resources(self, service: str, region: Optional[str] = None) -> List[Resource]:
        logger.info("[OnPremAdapter] list_resources: service=%s region=%s", service, region)
        return []  # TODO: integrate OnPrem SDK

    async def health_check(self) -> Dict[str, Any]:
        return {"provider": "onprem", "status": "stub", "message": "Not yet connected"}
