...
class GCPAdapter(BaseCloudAdapter):
    ...
    async def describe_resource(self, resource_id: str) -> Optional[Resource]:
        logger.info("[GCPAdapter] describe_resource: %s", resource_id)
        return None  # TODO: integrate GCP SDK

    async def list_resources(self, service: str, region: Optional[str] = None) -> List[Resource]:
        logger.info("[GCPAdapter] list_resources: service=%s region=%s", service, region)
        return []  # TODO: integrate GCP SDK

    async def health_check(self) -> Dict[str, Any]:
        return {"provider": "gcp", "status": "stub", "message": "Not yet connected"}
