...
class ISO27001Framework(ComplianceFrameworkDefinition):
    ...
    def get_version(self) -> FrameworkVersion:
        return FrameworkVersion(version="2022", year="2022")
        
    def map_findings(self, findings: List[SecurityFinding]) -> List[ComplianceRequirement]:
        reqs_map: Dict[str, ComplianceRequirement] = {
            "A.5": ComplianceRequirement(requirement_id="A.5", title="Organizational controls", description="Organizational policies.", status=ComplianceStatus.PASSED, controls=[]),
            "A.8": ComplianceRequirement(requirement_id="A.8", title="Technological controls", description="IT security controls.", status=ComplianceStatus.PASSED, controls=[])
        }
        
        for finding in findings:
            target_req = "A.8" 
            if finding.category == "GOVERNANCE": target_req = "A.5"
