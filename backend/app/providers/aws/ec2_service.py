...
class EC2Service:
    ...
    def describe_instances(self) -> List[Dict[str, Any]]:
        logger.debug("[EC2Service] Discovering EC2 instances...")
    ...
    def start_instance(self, instance_id: str):
        logger.debug(f"[EC2Service] Starting EC2 instance {instance_id}...")
        return self.client.start_instances(InstanceIds=[instance_id])

    def stop_instance(self, instance_id: str):
        logger.debug(f"[EC2Service] Stopping EC2 instance {instance_id}...")
        return self.client.stop_instances(InstanceIds=[instance_id])

    def reboot_instance(self, instance_id: str):
        logger.debug(f"[EC2Service] Rebooting EC2 instance {instance_id}...")
        return self.client.reboot_instances(InstanceIds=[instance_id])

    def terminate_instance(self, instance_id: str):
        logger.debug(f"[EC2Service] Terminating EC2 instance {instance_id}...")
