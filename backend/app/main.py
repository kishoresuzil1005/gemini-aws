...
from .models import (
    ScanHistoryDB, ResourceRelationshipDB, ResourceSnapshotDB, ResourceEdgeDB,
    CloudAccountDB, ResourceDB, ResourceNodeDB, DiscoveryResourceDB,
    UserDB, OrganizationDB, SavedMigrationDB, CloudIncidentDB,
    BackgroundJobDB, MetricDB, LogDB, AlertDB, CostReportDB,
    AutomationActionDB, PricingCacheDB, OptimizationRecommendationDB,
    AnomalyDB, BudgetDB, RemediationRequestDB
)
...
def get_all_recommendations(category: Optional[str] = None):
    engine = AIEngine()
    recs = engine.recommend(category=category)
    return {"count": len(recs), "recommendations": [r.dict() if hasattr(r, 'dict') else r for r in recs]}

@app.get("/api/v1/ai/recommendations/{resource_id}")
def get_resource_recommendations(resource_id: str):
    engine = AIEngine()
    recs = engine.recommend(resource_id=resource_id)
    return {"count": len(recs), "recommendations": [r.dict() if hasattr(r, 'dict') else r for r in recs]}

@app.get("/api/v1/ai/actions/remediation")
def get_all_remediation_plans():
    engine = AIEngine()
    plans = engine.plan_remediation()
    return {"count": len(plans), "plans": [p.dict() if hasattr(p, 'dict') else p for p in plans]}

@app.get("/api/v1/ai/actions/remediation/{resource_id}")
def get_resource_remediation(resource_id: str):
    engine = AIEngine()
    plans = engine.plan_remediation(resource_id=resource_id)
    return {"resource": resource_id, "count": len(plans), "plans": [p.dict() if hasattr(p, 'dict') else p for p in plans]}

@app.get("/api/v1/ai/actions/orchestration")
def get_all_orchestrations():
    engine = AIEngine()
    packages = engine.build_orchestration()
    return {"count": len(packages), "packages": [p.dict() if hasattr(p, 'dict') else p for p in packages]}

@app.get("/api/v1/ai/actions/orchestration/{resource_id}")
def get_resource_orchestration(resource_id: str):
    engine = AIEngine()
    packages = engine.build_orchestration(resource_id=resource_id)
    return {"resource": resource_id, "count": len(packages), "packages": [p.dict() if hasattr(p, 'dict') else p for p in packages]}

@app.post("/api/v1/ai/chat")
def ai_chat(request: ChatRequest, stream: bool = False):
    try:
        engine = AIEngine(ai_memory)
        response = engine.chat(request, stream=stream)
...
def ai_chat_stream(request: ChatRequest):
    try:
        engine = AIEngine(ai_memory)
        response = engine.chat(request, stream=True)
